...
ARGS_LIST = \
    ...
    "usr": GLOBAL_CONFIG['credentials']['ora'][0]['username'], # appliance username
    "pwd": GLOBAL_CONFIG['credentials']['ora'][0]['password'], # appliance password
...
def rackhdapi(url_cmd, action='get', payload=[], timeout=None, rest_headers={}):
    ...
    return restful("http://" + ARGS_LIST['ora'] + ":" + str(GLOBAL_CONFIG['ports']['http']) + url_cmd,
    ...
    if ARGS_LIST["nodeid"] != 'None':
        ...
        for nodeentry in catalog['json']:
            if ARGS_LIST["sku"] == 'all':
                ...
                if 'sku' in nodeentry and nodeentry['type'] == 'compute':
